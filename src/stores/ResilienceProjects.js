...
export const useProjectsStore = defineStore('projectsStore', () => {
  ...
  function whereValueOptions(whereAttribute) {
    const optionsLookup = {
      ...
      strategy: [
        ...
        { value: 'Acquire/Demolish', label: 'Acquire/Demolish' },
        { value: 'Create', label: 'Create' },
        { value: 'Create Floodplain', label: 'Create Floodplain' },
        { value: 'Create Floodplain Bench', label: 'Create Floodplain Bench' },
        { value: 'Elevate', label: 'Elevate' },
        { value: 'Enhance', label: 'Enhance' },
        { value: 'Enhance/Modify', label: 'Enhance/Modify' },
        { value: 'Increase Capacity', label: 'Increase Capacity' },
        { value: 'Modify', label: 'Modify' },
        { value: 'New Area', label: 'New Area' },
        { value: 'Nourish (Managed)', label: 'Nourish (Managed)' },
        { value: 'Nourish (New)', label: 'Nourish (New)' },
        { value: 'Nourish(Managed)', label: 'Nourish(Managed)' },
        { value: 'Other', label: 'Other' },
        { value: 'Re-align', label: 'Re-align' },
        { value: 'Remove Obstruction', label: 'Remove Obstruction' },
        { value: 'Replace in Kind', label: 'Replace in Kind' },
        { value: 'Replace with Other', label: 'Replace with Other' },
        { value: 'Restore', label: 'Restore' },
        { value: 'Restore (Direct Repair)', label: 'Restore (Direct Repair)' },
        { value: 'Restore (Tidal Flow)', label: 'Restore (Tidal Flow)' },
        { value: 'Restore(Direct Repair)', label: 'Restore(Direct Repair)' },
        { value: 'Undetermined', label: 'Undetermined' },
      ],
      town: [
      ...
      for (let i = 0; i < defExp.length; i++) {
        ...
        if (defExp[i].whereAttribute && defExp[i].whereValue) {
          def =
            def +
            '(' +
            defExp[i].whereAttribute.field +
            '=' +
            "'" +
            defExp[i].whereValue.value +
            "'" +
            ')'
  ...
  function applyDef(def) {
    ...
    layer.allLayers.items.forEach((sl) => (sl.definitionExpression = def))
  }
  return {
    defQuery,
    symbolizeBy,
